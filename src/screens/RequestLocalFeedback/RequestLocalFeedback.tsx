...
import { RequestLocalFeedbackProps } from "./types";
import {
  Box,
  Button,
  Divider,
...
const RequestLocalFeedback = ({ navigation }: RequestLocalFeedbackProps) => {
  const { createFeedbackLocal } = useFeedbacks();
  const {
    values,
    handleChange,
    handleBlur,
    ...
    initialValues: { name: "", address: "", type: "Local" },
    onSubmit: async () => {
  ...
  const onSubmit = async (props: IFeedback) => {
    try {
      await createFeedbackLocal(props);
      navigation.goBack();
      Toast.show({
...
export default RequestLocalFeedback;
